...
export function RequirementCard({ title, children, ellipse, footnote, className = "" }: RequirementCardProps) {
    return (
        <article
            className={`gradient-card relative rounded-2xl border border-gray-200 px-6 py-10 before:absolute before:top-0 before:left-1/2 before:block before:h-1 before:w-47.5 before:-translate-x-1/2 before:bg-white ${className}`}
        >
            {ellipse}
